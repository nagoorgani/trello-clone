...
export class MailService {
  ...
  constructor() {
    ...
    if (host && user && pass) {
      ...
      this.logger.warn('⚠️ SMTP credentials not configured. Emails will be logged to server console.');
    }
  }

  async sendPasswordResetEmail(to: string, resetCode: string): Promise<boolean> {
    const from = process.env.SMTP_FROM || '"Trello Clone Support" <noreply@trello.dev>';
    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0;">
        ...
        <div style="background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0; padding: 24px; text-align: center; margin: 24px 0;">
          <p style="color: #475569; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; margin-top: 0; margin-bottom: 12px;">Your 6-Digit Reset Code</p>
        ...
        <p style="color: #64748b; font-size: 13px; line-height: 1.6;">
          Enter this code on the password reset screen to set your new password. If you did not request a password reset, you can safely ignore this email.
        </p>

        <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
        <p style="color: #94a3b8; font-size: 11px; text-align: center; margin: 0;">
          © ${new Date().getFullYear()} Trello Clone Pro. All rights reserved.
        </p>
      </div>
    `;
